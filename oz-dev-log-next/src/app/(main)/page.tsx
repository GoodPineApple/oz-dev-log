...
export default function HomePage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const data = await fetchApi<ApiLog[]>(
        `/api/logs?userId=${encodeURIComponent(user.id)}`,
      );
      setLogs(data);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "일지를 불러오지 못했습니다.",
      );
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-zinc-500 dark:text-zinc-400">
          Next.js API Routes로 저장된 일지를 시간 역순으로 보여줍니다.
        </p>
      </div>
